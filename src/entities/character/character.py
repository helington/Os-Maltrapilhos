...
class Character(pygame.sprite.Sprite):    
    ...
    def load_animation_list(self, list_of_images_info):
        self.animation_list = {}
        for image_info in list_of_images_info:
            temp_list = []
    ...
    def move(self, game, world, follow_player):
        ...
        if self is follow_player and game.level == 0:
            game.world.screen_scroll = 0
            should_scroll = (
            ...
            if should_scroll:
                self.rect.x -= self.dx
                game.world.screen_scroll = -self.dx
    ...
    def draw(self, surface):
        # Fix for level transition when self.image might be a list instead of a Surface
        img = self.image[0] if isinstance(self.image, list) else self.image
